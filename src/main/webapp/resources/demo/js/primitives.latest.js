...
/* /common/init.js*/
/**
 * @preserve Basic Primitives Diagram v3.7.7
...
primitives.common.Graphics.prototype._activateLayer = function (layerName) {
	...
	if (layer === undefined) {
		...
		if (layerName === -1) {
			...
			canvas.addClass("Layer" + this.layerNames[layerName]);
...
primitives.common.ArrayReader.prototype.read = function (target, source, path, context) {
	...
	sequence = context.hash[sequencePath] || {};
	...
	for (index = 0, len = source.length; index < len; index += 1) {
		...
		if (!resultHash.hasOwnProperty(itemid)) {
			newHashObject = this.itemTemplate.read(hash[itemid] || {}, item, path + "-" + index, context);
...
/* /Controls/FamDiagram/models/CrossingsItem.js*/
...
/* /Controls/FamDiagram/models/FamilyEdge.js*/
primitives.famdiagram.FamilyEdge = function (parentid, childid) {
...
primitives.famdiagram.CursorNeighboursTask = function (cursorItemTask, neighboursSelectionModeOptionTask, navigationFamilyTask) {
	var _data = {
		items: []
	};
	...
	function process() {
		...
		_data.items = getCursorNeighbours(cursorTreeItemId, neighboursSelectionMode, navigationFamily);

		return true;
	}

	function getCursorNeighbours(cursorTreeItemId, neighboursSelectionMode, navigationFamily) {
		var result = [],
			processed = {},
			treeItem;
...
primitives.famdiagram.ItemsPositionsTask = function (currentControlSizeTask, scaleOptionTask, orientationOptionTask, itemsSizesOptionTask, connectorsOptionTask, visualTreeOptionTask,
	...
	function process() {
		...
		_logicalFamily = normalizeLogicalFamilyTask.getLogicalFamily();
		_treeLevels = orgTreeTask.getTreeLevels();
	...
	function getDiagramWidth() {
		...
		_treeLevels.loopLevels(this, function (levelIndex, level) {
			var levelLength = _treeLevels.getLevelLength(levelIndex);

			if (levelLength > 0) {
				var itemid = _treeLevels.getItemAtPosition(levelIndex, levelLength - 1),
					treeItemPosition = _data.treeItemsPositions[itemid];
		...
		_treeLevels.loopLevels(this, function (levelIndex, treeLevel) {
			...
			_treeLevels.loopLevelItems(this, levelIndex, function (treeItemId, treeItem, position) {
				var treeItemPosition = new primitives.orgdiagram.TreeItemPosition(),
					treeItemVisibility = combinedNormalVisibilityItemsTask.isItemSelected(treeItemId) ? 1/*primitives.common.Visibility.Normal*/ : (!treeItem.isVisible ? 4/*primitives.common.Visibility.Invisible*/ : 0/*primitives.common.Visibility.Auto*/),
	...
	function shiftLevels() {
		...
		for (index = 0, len = _data.treeLevelsPositions.length; index < len; index += 1) {
			...
			treeLevelConnectorStackSize = orgTreeTask.getConnectorsStacksSizes(index);
	...
	function setOffsets() {
		...
		var treeLevels = _treeLevels,
			family = _logicalFamily.getPlanarFamily(_treeLevels),
			alignmentGroups = [], alignmentGroup,
			alignmentGroupByParent = {};

		// create alignment groups
		index = 0,
		treeLevels.loopLevels(this, function (levelIndex, level) {
			treeLevels.loopLevelItems(this, levelIndex, function (itemid, item, position) {
				if (!alignmentGroupByParent.hasOwnProperty(itemid)) {
					var childrenCount = family.countChildren(itemid);
					if (childrenCount > 0) {
						var alignmentGroup = new AlignmentGroup(levelIndex, index);
						index += 1;
						alignmentGroup.addTop(position);
						family.loopChildren(this, itemid, function (childid, child) {
							alignmentGroup.addBottom(treeLevels.getItemPosition(childid));
							if (childrenCount == 1) {
								family.loopParents(this, childid, function (parentid) {
									alignmentGroup.addTop(treeLevels.getItemPosition(parentid));
									alignmentGroupByParent[parentid] = alignmentGroup;
									return family.SKIP;
								});
							}
							return family.SKIP;
						...
						alignmentGroupByParent[itemid] = alignmentGroup;
					}
				}
			});
		});

		var dependencyGroupsByItem = {},
			dependencyGroups = [],
			counter = 0,
			topMiddle,
			bottomMiddle;

		// create dependency groups out of alignment groups
		for (index = 0, len = alignmentGroups.length; index < len; index += 1) {
			alignmentGroup = alignmentGroups[index];
			topMiddle = alignmentGroup.topMiddle();
			bottomMiddle = alignmentGroup.bottomMiddle();
			if (topMiddle != null && bottomMiddle != null) {
				var topItem = treeLevels.getItemAtPosition(alignmentGroup.level, topMiddle),
					bottomItem = treeLevels.getItemAtPosition(alignmentGroup.level + 1, bottomMiddle),
					dependencyGroup = dependencyGroupsByItem[topItem];

				if (dependencyGroup == null) {
					dependencyGroup = new DependencyGroup(counter);
					counter += 1;
					dependencyGroup.add(alignmentGroup.level, topItem);
					dependencyGroupsByItem[topItem] = dependencyGroup;
					dependencyGroups.push(dependencyGroup);
				}
				dependencyGroup.add(alignmentGroup.level + 1, bottomItem);
				dependencyGroupsByItem[bottomItem] = dependencyGroup;
			}
		}

		// create dependency groups out of all other items
		treeLevels.loopLevels(this, function (levelIndex, level) {
			treeLevels.loopLevelItems(this, levelIndex, function (itemid, item, position) {
				var dependencyGroup = dependencyGroupsByItem[itemid];
				if (dependencyGroup == null) {
					dependencyGroup = new DependencyGroup(counter);
					counter += 1;
					dependencyGroup.add(levelIndex, itemid);
					dependencyGroupsByItem[itemid] = dependencyGroup;
					dependencyGroups.push(dependencyGroup);
				}
			});
		});

		var dependencies = new primitives.common.dependencies();
		// add horizontal depenedencies
		treeLevels.loopLevels(this, function (levelIndex, level) {
			var previous = null;
			treeLevels.loopLevelItems(this, levelIndex, function (itemid, item, position) {
				var dependencyGroup = dependencyGroupsByItem[itemid];
				if (previous != null) {
					dependencies.add(previous, dependencyGroup.id, new Dependency(null));
				}
				previous = dependencyGroup.id;
			});
		});


		//var crossEdges = new primitives.common.graph();
		// add criss cross alignment group dependencies
		for (index = 0, len = alignmentGroups.length; index < len; index += 1) {
			alignmentGroup = alignmentGroups[index];
			alignmentGroup.loopDependencies(function (fromPosition, fromLevel, fromIsMeasured, toPosition, toLevel, toIsMeasured) {
				var fromItem = treeLevels.getItemAtPosition(fromLevel, fromPosition),
					toItem = treeLevels.getItemAtPosition(toLevel, toPosition),
					fromGroup = dependencyGroupsByItem[fromItem],
					toGroup = dependencyGroupsByItem[toItem];

				dependencies.add(fromGroup.id, toGroup.id, new Dependency(fromIsMeasured ? fromItem : toItem));
			});
		}

		var groupOffsets = {},
			rightMargin = 0;
		dependencies.loopTopoForward(this, function (itemid, position) {
			var offset = 0;
			dependencies.loopBackward(this, itemid, function (from, edge) {
				var distanceTo = getDependencyGroupsDistance(dependencyGroups[from], dependencyGroups[itemid], edge.itemToMeasure);
				offset = Math.max(offset, groupOffsets[from] + distanceTo);
			});
			groupOffsets[itemid] = offset;

			rightMargin = Math.max(rightMargin, offset);
		});
		...
		for (index = 0, len = alignmentGroups.length; index < len; index += 1) {
			alignmentGroup = alignmentGroups[index];
			rightItem = null;
			alignmentGroup.getRightPosition(this, function (level, position) {
				var item = treeLevels.getItemAtPosition(level, position);
				fixedGroups[dependencyGroupsByItem[item].id] = true;
			});
		}

		dependencies.loopTopoBackward(this, function (itemid, position) {
			if (!fixedGroups.hasOwnProperty(itemid)) {
				var offset = null;
				dependencies.loopForward(this, itemid, function (to, edge) {
					var distanceTo = getDependencyGroupsDistance(dependencyGroups[itemid], dependencyGroups[to], edge.itemToMeasure);
					if (offset == null) {
						offset = groupOffsets[to] - distanceTo;
					} else {
						offset = Math.min(offset, groupOffsets[to] - distanceTo);
					}
				});
				if (offset != null) {
					groupOffsets[itemid] = offset;
		...
		var groupOffsets2 = {};
		dependencies.loopTopoBackward(this, function (itemid, position) {
			var offset = rightMargin;
			dependencies.loopForward(this, itemid, function (to, edge) {
				var distanceTo = getDependencyGroupsDistance(dependencyGroups[itemid], dependencyGroups[to], edge.itemToMeasure);
				offset = Math.min(offset, groupOffsets[to] - distanceTo);
			});
			groupOffsets2[itemid] = offset;
		});

		var fixedGroups2 = {};
		for (index = 0, len = alignmentGroups.length; index < len; index += 1) {
			alignmentGroup = alignmentGroups[index];
			alignmentGroup.getLeftPosition(this, function (level, position) {
				var item = treeLevels.getItemAtPosition(level, position);
				fixedGroups2[dependencyGroupsByItem[item].id] = true;
			});
		}

		dependencies.loopTopoForward(this, function (itemid, position) {
			if (!fixedGroups2.hasOwnProperty(itemid)) {
				position = null;
				dependencies.loopBackward(this, itemid, function (from, edge) {
					var distanceTo = getDependencyGroupsDistance(dependencyGroups[from], dependencyGroups[itemid], edge.itemToMeasure);
					if (position == null) {
						position = groupOffsets2[from] + distanceTo;
					} else {
						position = Math.max(position, groupOffsets2[from] + distanceTo);
					}
				});
				if (position != null) {
					groupOffsets2[itemid] = position;
				}
			}
		});

		var distancesReverted = {};
		for (var itemid in groupOffsets) {
			if (groupOffsets.hasOwnProperty(itemid)) {
				groupOffsets[itemid] = (groupOffsets[itemid] + groupOffsets2[itemid]) / 2.0;
			}
		}

		// fix left margin to 0
		var correction = null;
		dependencies.loopTopoForward(this, function (groupid, position, layer) {
			var dependencyGroup = dependencyGroups[groupid],
				groupOffset = groupOffsets[groupid];
			if (layer == 0) {
				dependencyGroup.loop(this, function (itemid) {
					var treeItemPosition = _data.treeItemsPositions[itemid],
						treeItemVisibility = treeItemPosition.actualVisibility,
						treeItemPadding = intervalsTask.getInterval(treeItemVisibility) / 2.0,
						itemOffset = groupOffset - treeItemPosition.actualSize.width / 2;

					correction = correction == null ? itemOffset : Math.min(itemOffset, correction);
				});
			} else {
				return true;
			}
		...
		// offset items inside of dependency groups
		var groupOffset;
		for (var groupIndex = 0, groupLen = dependencyGroups.length; groupIndex < groupLen; groupIndex += 1) {
			dependencyGroup = dependencyGroups[groupIndex];
			groupOffset = groupOffsets[dependencyGroup.id] || 0;
			...
			dependencyGroup.loop(this, function (itemid) {
				...
				treeItemPosition.offset = groupOffset - treeItemPosition.actualSize.width / 2 - correction;
			});
		}
	}

	function AlignmentGroup(level, id) {
		this.id = id;
		this.level = level;
		this.topLeft = null;
		this.topRight = null;
		...
		this.addTop = function (position) {
			this.topItems[position] = true;
			if (this.topLeft == null) {
				this.topLeft = position;
				this.topRight = position;
			} else {
				this.topLeft = Math.min(this.topLeft, position);
				this.topRight = Math.max(this.topRight, position);
			}
		};

		this.addBottom = function (position) {
			this.bottomItems[position] = true;
			if (this.bottomLeft == null) {
				this.bottomLeft = position;
				this.bottomRight = position;
			} else {
				this.bottomLeft = Math.min(this.bottomLeft, position);
				this.bottomRight = Math.max(this.bottomRight, position);
			}
		};
		...
		this.topMiddle = function () {
			...
			if (this.topLeft == this.topRight) {
				result = this.topLeft;
			} else if ((this.topRight - this.topLeft) % 2 === 0) {
				result = (this.topRight + this.topLeft) / 2;
				while (!this.topItems.hasOwnProperty(result)) {
					result += 1;
				}
			}
			return result;
		...
		this.bottomMiddle = function () {
			...
			} else if ((this.bottomRight - this.bottomLeft) % 2 === 0) {
				result = (this.bottomRight + this.bottomLeft) / 2;
				while (!this.bottomItems.hasOwnProperty(result)) {
					result += 1;
				}
			}
			return result;
		...
		this.loopDependencies = function (onDependency) { // onDependency = function(from, to)
			var topMiddle = this.topMiddle();
			var bottomMiddle = this.bottomMiddle();
			if (topMiddle == null) {
				var topLeft = Math.floor((this.topRight + this.topLeft) / 2.0);
				var topRight = Math.ceil((this.topRight + this.topLeft) / 2.0);

				onDependency(topLeft, this.level, true, bottomMiddle, this.level + 1, false);
				onDependency(bottomMiddle, this.level + 1, false, topRight, this.level, true);
			} else if (bottomMiddle == null) {
				var bottomLeft = Math.floor((this.bottomRight + this.bottomLeft) / 2.0);
				var bottomRight = Math.ceil((this.bottomRight + this.bottomLeft) / 2.0);

				onDependency(bottomLeft, this.level + 1, true, topMiddle, this.level, false);
				onDependency(topMiddle, this.level, false, bottomRight, this.level + 1, true);
			}
		};

		this.loopPositions = function (onPosition) { // onPosition = function(position, level)
			var position;
			for (position = this.topLeft; position <= this.topRight; position += 1) {
				onPosition(position, this.level);
			}
			for (position = this.bottomLeft; position <= this.bottomRight; position += 1) {
				onPosition(position, this.level + 1);
			}
		};

		this.getLeftPosition = function (thisArg, onPosition) {// onPosition = function(level, position)
			if (this.topLeft != this.topRight) {
				onPosition.call(thisArg, this.level, this.topLeft);
			} else if (this.bottomLeft != this.bottomRight) {
				onPosition.call(thisArg, this.level + 1, this.bottomLeft);
			}
		};

		this.getRightPosition = function (thisArg, onPosition) {// onPosition = function(level, position)
			if (this.topLeft != this.topRight) {
				onPosition.call(thisArg, this.level, this.topRight);
			} else if (this.bottomLeft != this.bottomRight) {
				onPosition.call(thisArg, this.level + 1, this.bottomRight);
			}
		};
	}

	function DependencyGroup(id) {
		this.id = id;
		this.items = {}; // items[levelIndex] = itemId;
		this.minimum = null;
		this.maximum = null;

		this.add = function (levelIndex, itemid) {
			this.items[levelIndex] = itemid;
			this.minimum = this.minimum != null ? Math.min(this.minimum, levelIndex) : levelIndex;
			this.maximum = this.maximum != null ? Math.max(this.maximum, levelIndex) : levelIndex;
		};

		this.loop = function (thisArg, onItem) { // onItem = function(itemid) {}
			var items = this.items;
			for (var levelIndex = this.minimum; levelIndex <= this.maximum; levelIndex += 1) {
				onItem.call(thisArg, items[levelIndex], levelIndex);
			}
		};
	}

	function Distances(source, offset) {
		this.offset = 0;
		this.items = {}; // items[itemid] = { fromid: distance, fromid2: distance2 }

		if (source != null) {
			this.items = source.items; // share items collection
			this.offset = source.offset + offset;
		}

		this.merge = function (source, offset) {
			var items = source.items,
				sourceOffset = source.offset + offset;
			for (var itemid in items) {
				if (items.hasOwnProperty(itemid)) {
					this.items[itemid] = Math.max(this.items[itemid] || 0, items[itemid] + sourceOffset);
				}
			}
		};

		this.loop = function (thisArg, onItem) { // onItem = function(itemid, distance) {}
			var items = this.items;
			for (var itemid in items) {
				if (items.hasOwnProperty(itemid)) {
					onItem.call(thisArg, itemid, items[itemid] + this.offset);
				}
			}
		};
	}
	...
	function getDependencyGroupsDistance(item1, item2, edgeItem) {
		var result = 0;
		if (edgeItem == null) {
			var minimum = Math.max(item1.minimum, item2.minimum),
				maximum = Math.min(item1.maximum, item2.maximum);
			for (var level = minimum; level <= maximum; level += 1) {
				result = Math.max(result, getItemsDistance(item1.items[level], item2.items[level]));
			}
		} else {
			result = getItemsDistance(edgeItem, null);
		}
		return result;
	}

	function getItemsDistance(itemid1, itemid2) {
		var result = 0;
		if (itemid1 != null) {
			var treeItemPosition1 = _data.treeItemsPositions[itemid1],
				treeItemVisibility1 = treeItemPosition1.actualVisibility,
				treeItemPadding1 = intervalsTask.getInterval(treeItemVisibility1) / 2.0,
				treeItemSize1 = treeItemPosition1.actualSize.width;
			result += treeItemSize1 / 2.0 + treeItemPadding1;
		}
		if (itemid2 != null) {
			var treeItemPosition2 = _data.treeItemsPositions[itemid2],
				treeItemVisibility2 = treeItemPosition2.actualVisibility,
				treeItemPadding2 = intervalsTask.getInterval(treeItemVisibility2) / 2.0,
				treeItemSize2 = treeItemPosition2.actualSize.width;
			result += treeItemSize2 / 2.0 + treeItemPadding2;
		}
		return result;
	}
...
/*  This task transforms normolized logical family into levels of nodes. Every node is connected only to parents and children located at neighbouring levels.
	This task is responsible to place nodes at levels in the most optimial way in terms of visualization. It is responsible to place parents close to its children and vise versa.
	It is responsible to minimize number of connectors intersections in graph.

	The current approach to optimize items placement is to transform family into hierarchy of nodes and order 
	nodes children in the way minimizing number of intersections between connection lines.
	1. Extract families into _families array of type FamilyItem. Family is sub tree of items _logicalFamily. 
		...
		The _orgTree collection is used to define final org hierarchy used to balance nodes in levels.
...
*/
primitives.famdiagram.LevelsTask = function (normalizeLogicalFamilyTask, defaultItemConfig, itemTemplateParamsTask) {
	var _data = {
		...
		bundles: null, /* array of primitives.orgdiagram.ConnectorBundle objects */
	...
	_orgTree, /*tree primitives.orgdiagram.OrgItem */
	_orgPartners, /* Creates extra partners collection of relations between visual tree items
	...
	function process() {
		...
		_orgTree = primitives.common.tree();
		...
		_orgTree.loopLevels(this, function (treeItemId, treeItem, levelIndex) {
			...
			if (templateParams != null && templateParams.isActive && treeItem.isVisible) {
		...
		recalcLevelsDepth(_data.bundles, _data.connectorStacks, _data.treeLevels, _orgTree, _orgPartners);
		...
		if (logicalFamily.hasNodes() > 0) {
			...
			for (index = 0, len = sortedFamilies.length; index < len; index += 1) {
				...
				for (index2 = 0; index2 < links.length; index2 += 1) {
					...
					toItem = _orgTree.node(link.toItem);
					fromItem = _orgTree.node(link.fromItem);
					...
					if (attachedFamilies[toItem.familyId] === true) {
						...
						if (rootItem.level >= familyRootItem.level) {
							_orgTree.loopParents(this, rootItem.id, function (nodeid, node) {
								rootItem = node;
			...
			balanceOrgTree(_orgTree, extraGravities, grandChildren);
		}
	}
	...
	function getGrandChildren() {
		...
		_orgTree.loopPostOrder(this, function (itemId, orgItem, parentId, parent) {
			var level;
	...
	function getExtraGravity() {
		...
		for (orgItemId in _orgPartners) {
			if (_orgPartners.hasOwnProperty(orgItemId)) {
				orgItem = _orgTree.node(orgItemId);
				extraPartners = _orgPartners[orgItemId];

				for (index = 0, len = extraPartners.length; index < len; index += 1) {
					extraPartner = _orgTree.node(extraPartners[index]);
	...
	function addExtraGravitiesForConnection(extraGravities, fromItem, toItem) {
		...
		_orgTree.zipUp(this, fromItem.id, toItem.id, function (fromItemId, parentFromItemId, toItemId, parentToItemId) {
	...
	function attachFamilyToOrgChart(parent, family) {
		...
		_orgTree.adopt(rootItem.id, familyRoot.id, familyRoot);
	}
	...
	function extractChildren(parentItem, logicalFamily, famItemsExtracted, family) {
		...
		if (_itemByChildrenKey[firstChild] != null) {
			...
			if (_orgPartners[partnerItem.id] == null) {
				_orgPartners[partnerItem.id] = [];
			}
			_orgPartners[partnerItem.id].push(parentItem.id);
	...
	function createOrgItem(id, parentId, familyId, level, userItem) {
		...
		_orgTree.add(parentId, orgItem.id, orgItem);
	...
	function recalcLevelsDepth(bundles, connectorStacks, treeLevels, orgTree, orgPartners) {
		...
		treeLevels.loopLevels(this, function (levelIndex, treeLevel) {
			...
			treeLevels.loopLevelItems(this, levelIndex, function (itemid, treeItem, position) {
				...
				if (!processed.hasOwnProperty(itemid)) {
					...
					if (fromItems.length > 1 || toItems.length > 0) {
						...
						bundle = new primitives.orgdiagram.ConnectorBundle();
						bundle.fromLevel = levelIndex;
						bundle.fromItems = fromItems;
						bundle.toItems = toItems;
			...
			if (bundlesToStack.length > 0) {
				...
				stacksSizes.parentsStackSize = stackSegments.resolve(this, function (from, to, bundle, offset) {
...
primitives.famdiagram.NormalizeLogicalFamilyTask = function (normalizeOptionTask, addSpousesTask) {
	...
	function normalize(logicalFamily, maximumId, debug) {
		...
		if (logicalFamily.hasNodes() > 0) {
			...
			/* enumerate */
			if (_normalizeOptions.alignBylevels) {
	...
	function resortItemsBylevels(logicalFamily) {
		...
		for (index = minimumLevel; index <= maximumLevel; index += 1) {
			...
			setLevelsForItems(itemsAtLevel, logicalFamily);
		}
	...
	function setLevelsForItems(items, logicalFamily) {
		var level = 0,
			key, famItem,
			nextItems;
...
primitives.famdiagram.Control = function (element, options) {
	...
	function createTaskManager(getOptions, getGraphics, getLayout) {
		...
		tasks.addTask('LevelsTask', ['NormalizeLogicalFamilyTask', 'defaultItemConfig', 'CombinedTemplateParamsTask'], primitives.famdiagram.LevelsTask, "#ff0000"/*primitives.common.Colors.Red*/);
		...
		tasks.addTask('ConnectionsGraphTask', ['graphics', 'CreateTransformTask', 'ConnectorsOptionTask', 'LevelsTask', 'AlignDiagramTask'], primitives.orgdiagram.ConnectionsGraphTask, "#00ffff"/*primitives.common.Colors.Cyan*/);

		// Transformations/Selections
		tasks.addTask('HighlightItemTask', ['HighlightItemOptionTask', 'LevelsTask'], primitives.orgdiagram.HighlightItemTask, "#00ffff"/*primitives.common.Colors.Cyan*/);

		tasks.addTask('CursorItemTask', ['CursorItemOptionTask', 'LevelsTask'], primitives.orgdiagram.CursorItemTask, "#00ffff"/*primitives.common.Colors.Cyan*/);
		tasks.addTask('CursorNeighboursTask', ['CursorItemTask', 'NeighboursSelectionModeOptionTask', 'AddLabelAnnotationsTask'], primitives.famdiagram.CursorNeighboursTask, "#00ffff"/*primitives.common.Colors.Cyan*/);
		...
		tasks.addTask('AlignDiagramTask', ['OrientationOptionTask', 'ItemsSizesOptionTask', 'VisualTreeOptionTask', 'ScaleOptionTask', 'PrintPreviewOptionTask', 'CurrentControlSizeTask', 'LevelsTask', 'ItemsPositionsTask', 'isFamilyChartMode'], primitives.orgdiagram.AlignDiagramTask, "#ff0000"/*primitives.common.Colors.Red*/);
		...
		// Managers
		tasks.addTask('BackgroundAnnotationManagerTask', ['ItemsSizesOptionTask', 'LevelsTask', 'AlignDiagramTask'], primitives.orgdiagram.BackgroundAnnotationManagerTask, "#00ffff"/*primitives.common.Colors.Cyan*/);
		...
		tasks.addTask('DrawTreeItemsTask', ['graphics', 'CreateTransformTask', 'ApplyLayoutChangesTask',
			...
			'AlignDiagramTask', 'CombinedTemplateParamsTask',
		...
		tasks.addTask('DrawItemLabelsTask', ['graphics', 'CreateTransformTask', 'ApplyLayoutChangesTask', 'LabelsOptionTask', 'LevelsTask', 'AlignDiagramTask'], primitives.orgdiagram.DrawItemLabelsTask, "#008000"/*primitives.common.Colors.Green*/);
...
/* /Controls/OrgDiagram/Models/ConnectorBundle.js*/
...
primitives.orgdiagram.TreeLevelConnectorStackSize = function () {
	...
	this.childrenStackSize = 0; /* number of overlapping horiontal connection lines between children in level */
};
...
primitives.orgdiagram.AlignDiagramTask = function (orientationOptionTask, itemsSizesOptionTask, visualTreeOptionTask, scaleOptionTask, printPreviewOptionTask,
	currentControlSizeTask, visualTreeLevelsTask, itemsPositionsTask, isFamilyChartMode) {
	var _data = {
		treeItemsPositions: {}, // primitives.orgdiagram.TreeItemPosition();
		treeLevelsPositions: [], // primitives.orgdiagram.TreeLevelPosition()
		panelSize: null // primitives.common.Rect();
	...
	function getTreeItemForMousePosition(x, y, gravityRadius) {
		...
		_activeTreeLevels.loopLevels(this, function (index, level) {
			...
			distance = Math.min(Math.abs(y - (treeLevelPosition.shift + treeLevelPosition.topConnectorShift)), Math.abs(y - (treeLevelPosition.shift + treeLevelPosition.connectorShift)));
		...
		if (bestIndex != null) {
			result = _activeTreeLevels.binarySearch(this, bestIndex, function (itemId) {
				var treeItemPosition = _data.treeItemsPositions[itemId];
				switch (treeItemPosition.actualVisibility) {
					case 1/*primitives.common.Visibility.Normal*/:
		...
		if (result != null) {
			var treeItemPosition = _data.treeItemsPositions[result];
			switch (treeItemPosition.actualVisibility) {
				case 1/*primitives.common.Visibility.Normal*/:
					break;
				default:
					if (Math.abs(x - treeItemPosition.actualPosition.horizontalCenter()) > Math.max(gravityRadius, treeItemPosition.actualPosition.width / 2.0) ||
						Math.abs(y - treeItemPosition.actualPosition.verticalCenter()) > Math.max(gravityRadius, treeItemPosition.actualPosition.height / 2.0)) {
						result = null;
					}
					break;
			}
...
primitives.orgdiagram.DrawItemLabelsTask = function (getGraphics, createTranfromTask, applyLayoutChangesTask,
	...
	function process() {
		...
		_graphics.reset("placeholder", 10/*primitives.common.Layers.Label*/);
	...
	function redrawLabels() {
		...
		_graphics.activate("placeholder", 10/*primitives.common.Layers.Label*/);
		attr = {
...
primitives.orgdiagram.CursorItemTask = function (cursorItemOptionTask, visualTreeLevelsTask) {
	var _data = {
	...
	function process() {
		var treeItemId = cursorItemOptionTask.getCursorItem(),
			activeTreeLevels = visualTreeLevelsTask.getActiveTreeLevels();

		_data.cursorTreeItemId = (treeItemId != null && activeTreeLevels.hasItem(treeItemId)) ? treeItemId : null;
...
primitives.orgdiagram.HighlightItemTask = function (highlightItemOptionTask, visualTreeLevelsTask) {
	var _data = {
	...
	function process() {
		...
		_data.highlightTreeItemId = (treeItemId != null && activeTreeLevels.hasItem(treeItemId)) ? treeItemId : null;
...
primitives.orgdiagram.BackgroundAnnotationManagerTask = function (itemsSizesOptionTask, visualTreeLevelsTask, alignDiagramTask) {
	var _data = {
	...
	function addTreeLevelPerimeters(perimeters, index, hashes, offsets, prevTreeLevelPosition, levelIndex, treeLevelPosition, nextTreeLevelPosition) {
		...
		_treeLevels.loopLevelItems(this, levelIndex, function (treeItemId, nextOrgItem) {
...
primitives.orgdiagram.ConnectionsGraphTask = function (getGraphics, createTranfromTask, connectorsOptionTask, visualTreeLevelsTask, alignDiagramTask) {
	...
	function process() {
		...
		_transform = createTranfromTask.getTransform();
		_connectorsOptions = connectorsOptionTask.getOptions();
		_bundles = visualTreeLevelsTask.getBundles();
		_treeLevels = visualTreeLevelsTask.getTreeLevels();
		_treeLevelsPositions = alignDiagramTask.getTreeLevelsPositions();
		_treeItemsPositions = alignDiagramTask.getItemsPositions();

		_data.graph = createConnectionsGraph(_bundles);

		return true;
	}

	function createConnectionsGraph(bundles) {
		var graph = primitives.common.graph(),
			panel = _graphics.activate("placeholder", 6/*primitives.common.Layers.Connector*/),
			index, len;

		_nodeid = 0;

		for (index = 0, len = bundles.length; index < len; index += 1) {
			var bundle = bundles[index];

			if (bundle.isVertical) {
				traceVerticalBundle(graph, bundle, panel.hasGraphics);
			} else {
				traceHorizontalBundle(graph, bundle);
			}
		}
	...
	function ConnectorEdge(from, to, polyline, parentsArrowId, childrenArrowId, dotId, weight, fromOffset, hasMiddle, middleParent) {
		this.polyline = polyline;
		this.from = from;
		this.to = to;
	...
	function traceHorizontalBundle(graph, bundle) {
		var fromItemId = bundle.fromItems[0],
			toItemId = bundle.toItems[0],
			fromItemPosition = _treeItemsPositions[fromItemId],
			toItemPosition = _treeItemsPositions[toItemId],
			treeLevelPosition = _treeLevelsPositions[bundle.fromLevel],
			polyline = new primitives.common.Polyline();

		if (fromItemPosition.actualPosition.x < toItemPosition.actualPosition.x) {
			_transform.transformPoints(fromItemPosition.actualPosition.right(), treeLevelPosition.shift + treeLevelPosition.horizontalConnectorsDepth,
				toItemPosition.actualPosition.x, treeLevelPosition.shift + treeLevelPosition.horizontalConnectorsDepth, true, this, function (fromX, fromY, toX, toY) {
			...
			_transform.transformPoints(fromItemPosition.actualPosition.x, treeLevelPosition.shift + treeLevelPosition.horizontalConnectorsDepth,
				toItemPosition.actualPosition.right(), treeLevelPosition.shift + treeLevelPosition.horizontalConnectorsDepth, true, this, function (fromX, fromY, toX, toY) {
					polyline.addSegment(new primitives.common.MoveSegment(fromX, fromY));
					polyline.addSegment(new primitives.common.LineSegment(toX, toY));
				});//ignore jslint
		}
		...
		graph.addEdge(fromItemId, toItemId, new ConnectorEdge(fromItemId, toItemId, polyline,
			toItemIsVisible ? toItemId : null, fromItemIsVisible ? fromItemId : null, (toItemIsVisible || fromItemIsVisible) ? 1000.0 : 10));
	}

	function ConnectorDestination(options) {
		this.id = null;
		this.x = null;
		this.y = null;
		this.bundleid = null;
	...
	function getId() {
		var result = "_" + _nodeid;
		_nodeid += 1;
		return result;
	}

	function traceVerticalBundle(graph, bundle, hasGraphics) {
		var parentTreeLevelPosition = _treeLevelsPositions[bundle.fromLevel],
			treeLevelPosition,
			connectorsStacksSizes = visualTreeLevelsTask.getConnectorsStacksSizes(bundle.fromLevel),
			points,
			...
			connectorPoint,
			connectorStep,
			chartHasSquaredConnectors = (_connectorsOptions.connectorType === 0/*primitives.common.ConnectorType.Squared*/),
			paletteItem, polyline;


		/* Find offset of horizontal connector line between partners */
		if (bundle.fromOffset > 0) {
			partnersConnectorOffset = parentTreeLevelPosition.shift + parentTreeLevelPosition.connectorShift -
				parentTreeLevelPosition.levelSpace / 2 * (connectorsStacksSizes.parentsStackSize - bundle.fromOffset + 1);
		} else {
			partnersConnectorOffset = parentTreeLevelPosition.shift + parentTreeLevelPosition.connectorShift;
		}
		...
		parents = [];
		if (bundle.fromItems.length > 0) {
			items = bundle.fromItems;
			for (index = 0, len = items.length; index < len; index += 1) {
				treeItemId = items[index];
				treeItemPosition = _treeItemsPositions[treeItemId];

				connectorPoint = new ConnectorDestination({
					id: treeItemId,
					bundleid: getId(),
		...
		children = [];
		if (bundle.toItems.length > 0) {
			hasSquared = false;

			items = bundle.toItems;
		...
		if (parents.length > 0) {
			topCenterPoint = new ConnectorDestination({
				id: getId(),
				x: parentHorizontalCenter,
				y: partnersConnectorOffset
			});
			traceTopConnectors(graph, topCenterPoint, parents, true, hasGraphics, true, bundle.fromOffset, _connectorsOptions.showExtraArrows && (children.length > 0));
		}

		var bottomCenterPoint = null;
		if (children.length > 0) {
			bottomCenterPoint = new ConnectorDestination({
				id: getId(),
				x: parentHorizontalCenter,
				y: (parentTreeLevelPosition.shift + parentTreeLevelPosition.connectorShift)
			});
			if (topCenterPoint != null && bottomCenterPoint.y == topCenterPoint.y) {
				bottomCenterPoint = topCenterPoint;
			}
			traceTopConnectors(graph, bottomCenterPoint, children, hasSquared, hasGraphics, false, 0, _connectorsOptions.showExtraArrows && (parents.length > 0));
		}
		...
		if (topCenterPoint != null && bottomCenterPoint != null && topCenterPoint.id != bottomCenterPoint.id) {
			_transform.transformPoints(topCenterPoint.x, topCenterPoint.y, bottomCenterPoint.x, bottomCenterPoint.y,
				true, this, function (fromX, fromY, toX, toY) {
					var polyline = new primitives.common.Polyline();
					polyline.addSegment(new primitives.common.MoveSegment(fromX, fromY));
					polyline.addSegment(new primitives.common.LineSegment(toX, toY));

					graph.addEdge(topCenterPoint.id, bottomCenterPoint.id, new ConnectorEdge(topCenterPoint.id, bottomCenterPoint.id, polyline, null, null, null, 0/* weight */));
				});
		}
	}

	function traceTopConnectors(graph, parentPoint, points, hasSquared, hasGraphics, isParents, fromOffset, showHorizontalArrows) {
		...
		if (hasSquared) {
			...
			if (len > 0) {
				...
				bundlePoint = (connectorDestination.x == parentPoint.x) ? parentPoint : new ConnectorDestination({
					id: connectorDestination.bundleid,
					x: connectorDestination.x,
					y: parentPoint.y
				});
				traceAngularConnectors(graph, bundlePoint, curvedPoints, false);
			}
			...
			for (index = startIndex; index <= endIndex; index += 1) {
				...
				bevelSize = _connectorsOptions.bevelSize;
				if (bevelSize < 2) {
					bevelSize = 0;
				}

				if (hasGraphics) {
					switch (_connectorsOptions.elbowType) {
				...
				if (connectorPoint.hasElbow) {
					...
					_transform.transformPoints(connectorPoint.x, parentPoint.y, connectorPoint.x, connectorPoint.y, true, this, function (fromX, fromY, toX, toY) {
						polyline.addSegment(new primitives.common.MoveSegment(fromX, fromY));
						polyline.addSegment(new primitives.common.LineSegment(toX, toY));
					}); //ignore jslint
				}
				...
				graph.addEdge(bundleid, connectorPoint.id, new ConnectorEdge(bundleid, connectorPoint.id, polyline,
					isParents && isVisible ? connectorPoint.id : null,
					!isParents && isVisible ? connectorPoint.id : null, null,
					isVisible ? 1000.0 : 10 /* weight*/, fromOffset));
			}
			...
			for (index = startIndex; index <= endIndex; index += 1) {
				...
				if (toPoint.x > fromPoint.x) {
					polyline = new primitives.common.Polyline();
					_transform.transformPoints(fromPoint.x, parentPoint.y, toPoint.elbowPoint2 != null ? toPoint.elbowPoint2.x : toPoint.x, parentPoint.y, true, this, function (startX, startY, endX, endY) {
					...
					graph.addEdge(fromPointId, toPointId, new ConnectorEdge(fromPointId, toPointId, polyline, null, null, fromPointId, 
			...
			traceAngularConnectors(graph, parentPoint, points, true);
		}
	}

	function traceAngularConnectors(graph, bundlePoint, points, drawToBundle) {
		var index, len,
			rect,
			point,
			polyline;
		...
		for (index = 0, len = points.length; index < len; index += 1) {
			...
			_transform.transformPoint(bundlePoint.x, bundlePoint.y, true, this, function (x, y) {
				polyline.addSegment(new primitives.common.MoveSegment(x, y));
			});//ignore jslint

			switch (_connectorsOptions.connectorType) {
			...
			graph.addEdge(bundlePoint.id, point.id, new ConnectorEdge(bundlePoint.id, point.id, polyline, null, isVisible ? point.id : null, isVisible ? 1000.0 : 10));
		}
	}
	...
	function recalcLevelsDepth(bundles, connectorStacks, treeLevels, orgTree, orgPartners) {
		...
		treeLevels.loopLevels(this, function (levelIndex, treeLevel) {
			...
			treeLevels.loopLevelItems(this, levelIndex, function (itemid, treeItem, position) {
				...
				if (treeItem.connectorPlacement & 8/*primitives.common.SideFlag.Left*/) {
					bundle = new primitives.orgdiagram.ConnectorBundle();
					bundle.isVertical = false;
					bundle.fromLevel = levelIndex;
					bundle.fromItems = [itemid];
					bundle.toItems = [treeLevels.getPrevItem(itemid)];
				...
				if (treeItem.connectorPlacement & 2/*primitives.common.SideFlag.Right*/) {
					...
					bundle.fromLevel = levelIndex;
					bundle.fromItems = [itemid];
					bundle.toItems = [treeLevels.getNextItem(itemid)];
...
primitives.common.GroupTitleTemplate = function (options) {
	...
	function render(event, data) {
		...
		config.orientation = options.groupTitleOrientation;
		config.verticalAlignment = options.groupTitleVerticalAlignment;
		config.horizontalAlignment = options.groupTitleHorizontalAlignment;

		config.text = itemConfig.groupTitle;
		config.fontSize = options.groupTitleFontSize;
		config.color = primitives.common.highestContrast(groupTitleColor, options.itemTitleSecondFontColor, options.itemTitleFirstFontColor);
		config.fontFamily = options.groupTitleFontFamily;
		config.fontWeight = options.groupTitleFontWeight;
		config.fontStyle = options.groupTitleFontStyle;
...
primitives.orgdiagram.Control = function (element, options) {
	...
	function createTaskManager(getOptions, getGraphics, getLayout) {
		...
		// Transformations/Selections
		tasks.addTask('HighlightItemTask', ['HighlightItemOptionTask', 'VisualTreeLevelsTask'], primitives.orgdiagram.HighlightItemTask, "#00ffff"/*primitives.common.Colors.Cyan*/);

		tasks.addTask('CursorItemTask', ['CursorItemOptionTask', 'VisualTreeLevelsTask'], primitives.orgdiagram.CursorItemTask, "#00ffff"/*primitives.common.Colors.Cyan*/);
		...
		tasks.addTask('AlignDiagramTask', ['OrientationOptionTask', 'ItemsSizesOptionTask', 'VisualTreeOptionTask', 'ScaleOptionTask', 'PrintPreviewOptionTask', 'CurrentControlSizeTask', 'VisualTreeLevelsTask', 'ItemsPositionsTask', 'isFamilyChartMode'], primitives.orgdiagram.AlignDiagramTask, "#ff0000"/*primitives.common.Colors.Red*/);
		tasks.addTask('CreateTransformTask', ['OrientationOptionTask', 'AlignDiagramTask'], primitives.orgdiagram.CreateTransformTask, "#00ffff"/*primitives.common.Colors.Cyan*/);
		tasks.addTask('CenterOnCursorTask', ['layout', 'CurrentControlSizeTask', 'CurrentScrollPositionTask', 'CursorItemTask', 'AlignDiagramTask', 'CreateTransformTask', 'ScaleOptionTask'], primitives.orgdiagram.CenterOnCursorTask, "#00ffff"/*primitives.common.Colors.Cyan*/);

		// Managers
		tasks.addTask('BackgroundAnnotationManagerTask', ['ItemsSizesOptionTask', 'VisualTreeLevelsTask', 'AlignDiagramTask'], primitives.orgdiagram.BackgroundAnnotationManagerTask, "#00ffff"/*primitives.common.Colors.Cyan*/);
...
primitives.common.dependencies = function () {
	...
	function add(from, to, edge) {
		if (from == null || to == null) {
			throw "Node id should not be null!";
		}
		if ((_forwardEdges[from] == null || _forwardEdges[from][to] == null) && edge != null) {

			if (_forwardEdges[from] == null) {
				_forwardEdges[from] = {};
				_forwardCount[from] = 0;
			}
			if (_forwardEdges[from][to] == null) {
				_forwardEdges[from][to] = edge;
				_forwardCount[from] += 1;
			}

			if (_backwardEdges[to] == null) {
				_backwardEdges[to] = {};
				_backwardCount[to] = 0;
			}
			if (_backwardEdges[to][from] == null) {
				_backwardEdges[to][from] = edge;
				_backwardCount[to] += 1;
			}
		}
	}
	...
	function dependency(from, to) {
		var result = null;
		if (_forwardEdges[from] != null && _forwardEdges[from][to]) {
			result = _forwardEdges[from][to];
		}
		return result;
	}

	function dependencyBackward(from, to) {
		var result = null;
		if (_backwardEdges[from] != null && _backwardEdges[from][to]) {
			result = _backwardEdges[from][to];
		}
		return result;
	}

	function countForward(from) {
		return _forwardCount[from] || 0;
	}

	function countBackword(from) {
		return _backwardCount[from] || 0;
	}

	function _loop(thisArg, collection, from, onEdge) {
		var itemid,
			items = collection[from];
		if (items != null) {
			for (itemid in items) {
				if (items.hasOwnProperty(itemid)) {
					if (onEdge.call(thisArg, itemid, items[itemid])) {
						break;
					}
				}
			}
	...
	function loopForward(thisArg, from, onEdge) { // onItem(to, edge)
		if (onEdge != null) {
			if (from != null) {
				_loop(thisArg, _forwardEdges, from, onEdge);
			}
		}
	}

	function loopBackward(thisArg, to, onEdge) { // onItem(from, edge)
		if (onEdge != null) {
			if (to != null) {
				_loop(thisArg, _backwardEdges, to, onEdge);
			}
		}
	}

	function firstForward(from) {
		var result = null;
		if (from != null) {
			_loop(this, _forwardEdges, from, function (to, edge) {
				result = to;
				return true;
			});
		}
	...
	function firstBackward(to) {
		var result = null;
		if (to != null) {
			_loop(this, _backwardEdges, to, function (from, edge) {
				result = from;
				return true;
			});
		}
	...
	function _loopTopo(thisArg, forwardCol, forwardCount, backwardCol, backwardCount, onItem) { // onItem(itemid, position, layer)
		var index, len, nodeid, references,
			queue, newQueue, position, layer;

		if (onItem != null) {
			/* count parents for every node */
			queue = [];
			references = {};
			for (nodeid in backwardCount) {
				if (backwardCount.hasOwnProperty(nodeid)) {
					references[nodeid] = backwardCount[nodeid];
				}
			}
			for (nodeid in forwardCol) {
				if (forwardCol.hasOwnProperty(nodeid)) {
					if (!references.hasOwnProperty(nodeid)) {
						references[nodeid] = 0;
						queue.push(nodeid);
					}
				}
			...
			while (queue.length > 0) {
				...
				for (index = 0, len = queue.length; index < len; index += 1) {
					...
					if (onItem.call(thisArg, nodeid, position, layer)) {
						newQueue = [];
						break;
					}
	...
	function loopTopoForward(thisArg, onItem) { // onItem(itemid, position)
		_loopTopo(thisArg, _forwardEdges, _forwardCount, _backwardEdges, _backwardCount, onItem);
	}

	function loopTopoBackward(thisArg, onItem) { // onItem(itemid, position)
		_loopTopo(thisArg, _backwardEdges, _backwardCount, _forwardEdges, _forwardCount, onItem);
	}

	return {
		add: add,
		dependency: dependency,
		dependencyBackward: dependencyBackward,
		countForward: countForward,
		countBackword: countBackword,
		loopForward: loopForward,
		loopBackward: loopBackward,
		firstForward: firstForward,
		firstBackward: firstBackward,
		loopTopoForward: loopTopoForward,
		loopTopoBackward: loopTopoBackward
	};
};
...
primitives.common.TreeLevels = function (source) {
	...
	function getItemPosition(itemid) {
		return _items.hasOwnProperty(itemid) ? _items[itemid].position : null;
	}
