...
@ManagedBean
@ViewScoped
public class ControladorArchivoVisitas implements Serializable{

    private static final long serialVersionUID = 4010381717041597330L;
 //    Variables de Lectura
    ...
    @Inject
    ControladorVisitasIndustriales controladorVisitasIndustriales;
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            System.out.println("mx.edu.utxj.pye.siip.controller.vin.ControladorArchivoVisitas.consultaAreaRegistro(): " + e.getMessage());
        }
    }
    ...
    //ActionListener
    public void subirExcelVisitas() {
        if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"visitas_industriales",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorVisitasIndustriales.listaVisitasIndustrialesPrevia(rutaArchivo);
                rutaArchivo = null;
            } else {
                rutaArchivo = null;
