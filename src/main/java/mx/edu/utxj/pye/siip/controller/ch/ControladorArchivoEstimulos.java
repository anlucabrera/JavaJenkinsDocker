...
@ManagedBean
@ViewScoped
public class ControladorArchivoEstimulos implements Serializable{

    private static final long serialVersionUID = 2169588687259618734L;
    ...
    @Inject ControladorProgramasEstimulos controladorProgramasEstimulos;
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            System.out.println("ControladorArchivoEstimulos.consultaAreaRegistro: " + ex.getMessage());
        }
    }
    ...
    //ActionListener
    public void subirExcelProgEst() throws IOException {
         if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"programas_estimulos",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorProgramasEstimulos.listaProgramasEstimulosPrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
