...
@ManagedBean
@ViewScoped
public class ControladorArchivoServiciosTecnologicosAnioMes implements Serializable{

    private static final long serialVersionUID = -6362686946002620947L;
    ...
    @Inject
    ControladorServiciosTecnologicosAnioMes controladorServiciosTecnologicosAnioMes;
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            System.out.println("mx.edu.utxj.pye.siip.controller.vin.ControladorArchivoServiciosTecnologicosAnioMes.consultaAreaRegistro(): " + e.getMessage());
        }
    }
    ...
    public void subirExcelServiciosTecnologicos() throws IOException{
        if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"servicios_tecnologicos",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorServiciosTecnologicosAnioMes.listaServiciosTecnologicosAnioMesPrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
