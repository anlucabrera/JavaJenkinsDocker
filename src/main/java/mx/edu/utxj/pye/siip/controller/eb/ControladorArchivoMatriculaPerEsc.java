...
@ManagedBean
@ViewScoped
public class ControladorArchivoMatriculaPerEsc implements Serializable{

    private static final long serialVersionUID = -1001778528753062348L;
    ...
    @Inject
    ControladorMatriculaPeriodosEscolares controladorMatriculaPeriodosEscolares;
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            System.out.println("mx.edu.utxj.pye.siip.controller.eb.ControladorArchivoMatriculaPerEsc.consultaAreaRegistro(): " + e.getMessage());
        }
    }
    ...
    public void subirExcelMatriculaPeriodoEscolar() throws IOException {
        if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio), String.valueOf(area.getSiglas()), eje,ejbModulos.getEventoRegistro().getMes(), "matricula_inicial", file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorMatriculaPeriodosEscolares.listaMatriculaPeriodosEscolaresPrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
