...
@ManagedBean
@ViewScoped
public class ControladorArchivoBolsa implements Serializable{
    ...
    @Inject ControladorBolsaEntrevistas controladorBolsaEntrevistas;
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            System.out.println("mx.edu.utxj.pye.siip.controller.vin.ControladorArchivoBolsa.consultaAreaRegistro(): " + e.getMessage());
        }
    }
    ...
    //ActionListener
    public void subirExcelBolsa() throws IOException{
         
        if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"bolsa_trabajo",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorBolsaTrabajo.listaBolsaTrabajoPrevia(rutaArchivo);
                controladorBolsaEntrevistas.listaBolsaEntrevistasPrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
