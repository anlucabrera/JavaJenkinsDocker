...
@ManagedBean
@ViewScoped
public class ControladorArchivoReconocimiento implements Serializable{

    private static final long serialVersionUID = 494840929795060549L;
    ...
    @Inject ControladorReconocimientoProdep controladorReconocimientoProdep;
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            System.out.println("mx.edu.utxj.pye.siip.controller.ch.ControladorArchivoReconocimiento.consultaAreaRegistro(): " + e.getMessage());
        }
    }
    ...
    //ActionListener
    public void subirExcelRecProdep() throws IOException {
       if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"reconocimientos_prodep",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorReconocimientoProdep.listaRecocimientoProdepPrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
