...
@ManagedBean
@ViewScoped
public class ControladorArchivoEstadiasPorEstudiante implements Serializable{

    private static final long serialVersionUID = 3247075996788638566L;
    ...
    @Inject ControladorEstadiasPorEstudiante controladorEstadiasPorEstudiante;
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            System.out.println("ControladorArchivoEstadiasPorEstudiante.consultaAreaRegistro: " + ex.getMessage());
        }
    ...
    public void subirExcelEstadiasPorEstudiante() throws IOException{
        if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"estadias_estudiante",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorEstadiasPorEstudiante.listaEstadiasPorEstudiantePrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
