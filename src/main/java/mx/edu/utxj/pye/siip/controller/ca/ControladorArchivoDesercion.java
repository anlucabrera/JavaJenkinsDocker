...
@ManagedBean
@ViewScoped
public class ControladorArchivoDesercion implements Serializable{

    private static final long serialVersionUID = -5206868580865258032L;
   //    Variables de Lectura
    ...
    @Inject
    ControladorDesercionPeriodos controladorDesercionPeriodos;
    
    @Inject
    ControladorDesercionReprobacion controladorDesercionReprobacion;
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            System.out.println("ControladorArchivoDesercion.consultaAreaRegistro " + ex.getMessage());
        }
    }
    ...
    //ActionListener
    public void subirExcelDesercion() {
         if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"desercion_academica",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorDesercionPeriodos.listaDesercionPeriodosPrevia(rutaArchivo);
                controladorDesercionReprobacion.listaDesercionReprobacionPrevia(rutaArchivo);
                rutaArchivo = null;
            } else {
                rutaArchivo = null;
