...
@ManagedBean
@ViewScoped
public class ControladorArchivoEgresados implements Serializable{

    private static final long serialVersionUID = 3554094473412167564L;
    ...
    @Inject ControladorEgresados controladorEgresados;
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            System.out.println("mx.edu.utxj.pye.siip.controller.vin.ControladorArchivoEgresados.consultaAreaRegistro(): " + e.getMessage());
        }
    }
    ...
    public void subirExcelEgresados() throws IOException{
        if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"egresados",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorEgresados.listaEgresadosPrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
