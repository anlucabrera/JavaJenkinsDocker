...
@ManagedBean
@ViewScoped
public class ControladorArchivoExani implements Serializable{
    
    private static final long serialVersionUID = -8746099208734611605L;
    ...
    //ActionListener
    public void subirExcelExani() throws IOException{
        
       if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"exani",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorExaniResultados.listaExaniResultadosPrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
