...
@ManagedBean
@ViewScoped
public class ControladorArchivoEvaluacionSatEduContAnioMes implements Serializable{

    private static final long serialVersionUID = -2062359791222609852L;
    ...
    @Inject
    ControladorEvaluacionSatEduContAnioMes controladorEvaluacionSatEduContAnioMes;
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            areaRegistro = controladorModulosRegistro.consultaAreaRegistro((short) 55);
    ...
    public void subirExcelEvaluacionSatEduCont() throws IOException{
        if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"evaluacion_satisfaccion_educont",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorEvaluacionSatEduContAnioMes.listaEvaluacionSatEduContAnioMesPrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
