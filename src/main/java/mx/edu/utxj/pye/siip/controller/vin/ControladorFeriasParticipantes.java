...
//            Messages.addGlobalInfo("No existen registros");
    ...
    public void guardaFeriasprofParticipantes() {
         try {
             ejbFeriasParticipantes.guardaFeriasParticipantes(dto.getListaFeriasParticipantes(), dto.getRegistroTipo(), dto.getEjesRegistro(), dto.getArea(), controladorModulosRegistro.getEventosRegistros());
        } catch (Throwable ex) {
            Messages.addGlobalFatal("Ocurrió un error (" + (new Date()) + "): " + ex.getCause().getMessage());
            Logger.getLogger(ControladorFeriasParticipantes.class.getName()).log(Level.SEVERE, null, ex);
        }
