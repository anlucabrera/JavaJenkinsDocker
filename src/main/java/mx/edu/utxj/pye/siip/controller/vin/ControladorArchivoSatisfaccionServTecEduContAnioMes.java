...
@ManagedBean
@ViewScoped
public class ControladorArchivoSatisfaccionServTecEduContAnioMes implements Serializable{

    private static final long serialVersionUID = -2402577908185492017L;
    ...
    @Inject
    ControladorSatisfaccionServTecEduContAnioMes controladorSatisfaccionServTecEduContAnioMes;
    ...
    public void subirExcelSatisfaccionServTecEduCont() throws IOException{
        if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"satisfaccion_servtec_educont",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorSatisfaccionServTecEduContAnioMes.listaSatisfaccionServTecEduContAnioMesPrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
