...
@ManagedBean
@ViewScoped
public class ControladorArchivoProgramasPertCalidad implements Serializable{
    ...
    @Inject
    ControladorProgramasPertCalidad controladorProgramasPertCalidad;
    ...
    //ActionListener
    public void subirExcelProgramas() {
        
       if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistro(String.valueOf(ejercicio),String.valueOf(area),eje,"programas_pertcalidad",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                controladorProgramasPertCalidad.listaProgramasPertCalidadPrevia(rutaArchivo);
                rutaArchivo = null;
            } else {
                rutaArchivo = null;
