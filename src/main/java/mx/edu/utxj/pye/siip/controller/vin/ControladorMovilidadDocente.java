...
public class ControladorMovilidadDocente implements Serializable{
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            System.out.println("mx.edu.utxj.pye.siip.controller.vin.ControladorMovilidadDocente.consultaAreaRegistro(): " + e.getMessage());
        }
    }
    ...
    public void listaMovilidadDocentePrevia(String rutaArchivo) {
       try {
            if(rutaArchivo != null){
                ...
                dto.setLista(ejb.getListaMovilidadDocente(rutaArchivo));
            }
        } catch (Throwable ex) {
    ...
    public void guardaMovilidadDocente() {
       if (dto.getLista() != null) {
            try {
                ejb.guardaMovilidadDocente(dto.getLista(), dto.getRegistroTipo(), dto.getEje(), dto.getArea(), controladorModulosRegistro.getEventosRegistros());
            } catch (Throwable ex) {
                Messages.addGlobalFatal("Ocurrió un error (" + (new Date()) + "): " + ex.getCause()!=null?ex.getCause().getMessage():ex.getMessage());
                Logger.getLogger(ControladorRegistrosMovilidad.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void seleccionarRegistro(DTOMovilidadDocente registro){
     ...
     public void eliminarAlineacion(){
        Boolean eliminado = ejbEvidenciasAlineacion.eliminarAlineacion(dto.getRegistro().getRegistroMovilidadDocente().getRegistro());
        if(eliminado){ 
            Messages.addGlobalInfo("La alineación se eliminó de forma correcta.");
            dto.getRegistro().setActividadAlineada(null);
            dto.setAlineacionActividad(ejbEvidenciasAlineacion.getActividadAlineada(dto.getRegistro().getRegistroMovilidadDocente().getRegistro()));
            actualizarEjes();
            cargarAlineacionXActividad();
            Ajax.update("frmAlineacionDoc");
    ...
    public void cargarAlineacionXActividad(){
     ...
     public void alinearRegistro(){
        Boolean alineado = ejbEvidenciasAlineacion.alinearRegistroActividad(dto.getAlineacionActividad(), dto.getRegistro().getRegistroMovilidadDocente().getRegistro());
        if(alineado){
            controladorRegistrosMovilidad.cargarListaPorEvento();
     ...
     public void abrirAlineacionPOA(DTOMovilidadDocente registro){
        dto.setRegistro(registro);        
        dto.setAlineacionActividad(ejbEvidenciasAlineacion.getActividadAlineada(registro.getRegistroMovilidadDocente().getRegistro()));
        actualizarEjes();
        cargarAlineacionXActividad();
        Ajax.update("frmAlineacionDoc");
        Ajax.oncomplete("skin();");
        Ajax.oncomplete("PF('modalAlineacionDoc').show();");
    }
