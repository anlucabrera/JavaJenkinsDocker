...
@ManagedBean
@ViewScoped
public class ControladorArchivoDistribucionInstalaciones implements Serializable{

    private static final long serialVersionUID = 2579765469989669727L;
    ...
    @Inject ControladorDistribucionInstalaciones controladorDistribucionInstalaciones;
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            System.out.println("mx.edu.utxj.pye.siip.controller.eb.ControladorArchivoDistribucionInstalaciones.consultaAreaRegistro(): " + e.getMessage());
        }
    }
    ...
    public void subirExcelDistribucionInstalaciones() throws IOException{
        if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"distribucion_instalaciones",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorDistribucionInstalaciones.listaDistribucionInstalacionesPrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
