...
@ManagedBean
@ViewScoped
public class ControladorArchivoPerCap implements Serializable{

    private static final long serialVersionUID = 4836988265428481187L;
    ...
    @Inject ControladorPerCapParticipantes controladorPerCapParticipantes;
    ...
    public void consultaAreaRegistro() {
        try {
            ...
            System.out.println("mx.edu.utxj.pye.siip.controller.ch.ControladorArchivoPerCap.consultaAreaRegistro(): " + e.getMessage());
        }
    }
    ...
    //ActionListener
    public void subirExcelPerCap() throws IOException {
        if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"personal_capacitado",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorPersonalCapacitado.listaPersonalCapacitadoPrevia(rutaArchivo);
                controladorPerCapParticipantes.listaParticipantesPerCapPrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
