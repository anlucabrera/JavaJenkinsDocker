...
@ManagedBean
@ViewScoped
public class ControladorArchivoProgPertCal implements Serializable{

    private static final long serialVersionUID = 3311183198124345922L;
    ...
    @Inject ControladorProgramasPertCalidad controladorProgramasPertCalidad;
    ...
    //ActionListener
    public void subirExcelProgPertCal() throws IOException {
       if (file != null) {
            rutaArchivo = ejbCarga.subirExcelRegistroMensual(String.valueOf(ejercicio),String.valueOf(area.getSiglas()),eje,ejbModulos.getEventoRegistro().getMes(),"programas_PertCalidad",file);
            if (!"Error: No se pudo leer el archivo".equals(rutaArchivo)) {
                setEtapa(RegistroSiipEtapa.CARGAR);
                controladorProgramasPertCalidad.listaProgPertCalPrevia(rutaArchivo);
                rutaArchivo = null;
                file.delete();
            } else {
                rutaArchivo = null;
