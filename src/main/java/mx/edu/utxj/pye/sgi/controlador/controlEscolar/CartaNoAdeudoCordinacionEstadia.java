...
import mx.edu.utxj.pye.sgi.dto.controlEscolar.CartaNoAdeudoRolCordinacionEstadia;
...
@Named
@ViewScoped
public class CartaNoAdeudoCordinacionEstadia extends ViewScopedRol implements Desarrollable {
    ...
    @Getter @Setter
    CartaNoAdeudoRolCordinacionEstadia rol;
    ...
    public void init(){
        ...
        cargado = true;
        try{
            setVistaControlador(ControlEscolarVistaControlador.CARTA_NO_ADUEDO_CORDINACION_ESTADIA);
...
//            System.out.println("resValidacion = " + resValidacion);
            ...
            PersonalActivo cordinador = filtro.getEntity();//ejbPersonalBean.pack(logon.getPersonal());
            rol = new CartaNoAdeudoRolCordinacionEstadia(filtro, cordinador);
            tieneAcceso = rol.tieneAcceso(cordinador);
...
//            System.out.println("tieneAcceso2 = " + tieneAcceso);
            ...
            rol.setCordinadorEstadia(cordinador);
            // ----------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public void  getCatalogoArea(){
        try{
            ResultadoEJB<List<CatalogoNoAdeudoArea>> resCatalogo= ejb.getCatalogoAduedobyArea(AreaCartaNoAdeudo.CORDINACION_ESTADIA);
    ...
    public void getListaDtoNoAdeudo(){
        try {
           // System.out.println("CartaNoAdeudoDireccionCarrera.getListaDtoNoAdeudo Generacionseleccionada "+ rol.getGeneracionSelect());
            ...
            ResultadoEJB<List<DtoNoAdeudoEstudiante.NoAdeudoEstudianteGeneral>> resList= ejb.packEstadiaList(rol.getGeneracionSelect(),rol.getNivelEstudios(), AreaCartaNoAdeudo.CORDINACION_ESTADIA,rol.getCordinadorEstadia().getPersonal());
...
//        Object newValue = event.getNewValue();
        ...
        ResultadoEJB<DtoNoAdeudoEstudiante.CordinacionEstadia> resUpdate= ejb.updateNoAdeudoEstadia(registroNew.getCordinacionEstadia(),rol.getCordinadorEstadia().getPersonal());
    ...
    public void updateCarta(){
        try{
            rol.getEstudiantes().stream().forEach(e->{
                ResultadoEJB<DtoNoAdeudoEstudiante.CordinacionEstadia> resUpdate= ejb.updateNoAdeudoEstadia(e.getCordinacionEstadia(),rol.getCordinadorEstadia().getPersonal());
