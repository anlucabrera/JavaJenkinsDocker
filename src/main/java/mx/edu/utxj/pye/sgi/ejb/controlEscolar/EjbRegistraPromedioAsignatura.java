...
//            System.out.println("verificarRegistro = " + verificarRegistro);
     ...
     * @return Regresa un Point con la coordenada X indicando el total de promedios posibles y la coordenada Y indicando los que se pudieron registrar
     */
    public ResultadoEJB<Point> registrarMasivamentePromediosAlineacion(){
        try{
            System.out.println("EjbRegistraPromedioAsignatura.registrarMasivamentePromediosAlineacion");
            ResultadoEJB<Integer> verificarRegistro = verificarRegistro();
//            System.out.println("verificarRegistro = " + verificarRegistro);
            if(!verificarRegistro.getCorrecto()) return ResultadoEJB.crearErroneo(3, verificarRegistro.getMensaje(), Point.class);
            final List<ResultadoEJB<BigDecimal>> resultadoEJBS = new Vector<>();
                    ...
                    dtoEstudiantes.parallelStream().forEach(dtoEstudiante -> {
                        ...
                        if(getConfiguraciones.getCorrecto()){
                            ...
                            if(packDtoUnidadesCalificacion.getCorrecto()){
                                ...
                                resultadoEJBS.add(promediarAsignatura);
                            }else System.out.println("Error: EjbRegistraPromedioAsignatura.registrarMasivamentePromediosAlineacion proceso > packDtoUnidadesCalificacion = " + packDtoUnidadesCalificacion);
                        }else System.out.println("Error: EjbRegistraPromedioAsignatura.registrarMasivamentePromediosAlineacion proceso > getConfiguraciones = " + getConfiguraciones);
                    });
                }else System.out.println("Error: EjbRegistraPromedioAsignatura.registrarMasivamentePromediosAlineacion proceso > packDtoEstudiantesGrupo = " + packDtoEstudiantesGrupo);
                cargasCalculadas.add(dtoCargaAcademica);
            });
            Integer calculados = (int)resultadoEJBS.stream().filter(ResultadoEJB::getCorrecto).count();
            Point resultado = new Point(resultadoEJBS.size(), calculados);
            ...
            e.printStackTrace();
            return ResultadoEJB.crearErroneo(1, "Ocurrió un error al intentar ejecutar de forma masiva el registro de promedios por asignatura (EjbRegistraPromedioAsignatura.registrarMasivamentePromediosAlineacion).", e, Point.class);
        }
    }
