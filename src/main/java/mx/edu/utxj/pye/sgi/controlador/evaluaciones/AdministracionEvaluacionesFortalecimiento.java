...
@Named
@ViewScoped
public class AdministracionEvaluacionesFortalecimiento extends ViewScopedRol implements Desarrollable {
    @EJB private EjbPropiedades ep;
    @EJB private EjbAdministracionEvaluaciones ejdAdminEvaluaciones;
    @Getter @Setter private AdministracionEvaluacionesRolFortalecimiento rol;
    ...
    public void init(){
        ...
        cargado = true;
        try{
            setVistaControlador(ControlEscolarVistaControlador.ADMINISTRACION_EVALUACIONES_FORTALECIMIENTO);
            ResultadoEJB<Filter<PersonalActivo>> resAcceso = ejdAdminEvaluaciones.validaPersonalFortalecimiento(logonMB.getPersonal().getClave()); //Validar si pertenece departamento de Fortalecimeinto academico
            if(!resAcceso.getCorrecto()){ mostrarMensajeResultadoEJB(resAcceso);return;}//cortar el flujo si no se pudo verificar el acceso

            ResultadoEJB<Filter<PersonalActivo>> resValidacion = ejdAdminEvaluaciones.validaPersonalFortalecimiento(logonMB.getPersonal().getClave());

            if(!resValidacion.getCorrecto()){ mostrarMensajeResultadoEJB(resValidacion);return; }//cortar el flujo si no se pudo validar

            Filter<PersonalActivo> filtro = resValidacion.getValor();//se obtiene el filtro resultado de la validación
            PersonalActivo personalFortalecimiento = filtro.getEntity();//            ejbPersonalBean.pack(logonMB.getPersonal());
            rol = new AdministracionEvaluacionesRolFortalecimiento(filtro, personalFortalecimiento, personalFortalecimiento.getAreaOperativa());
            tieneAcceso = rol.tieneAcceso(personalFortalecimiento);
            if(!tieneAcceso){mostrarMensajeNoAcceso(); return;} //cortar el flujo si no tiene acceso
            rol.setPersonalPsicopedagogia(personalFortalecimiento);
            // ----------------------------------------------------------------------------------------------------------------------------------------------------------
