...
import mx.edu.utxj.pye.sgi.dto.controlEscolar.CartaNoAdeudoRolRecursosMateriales;
...
@Named
@ViewScoped
public class CartaNoAdeudoRecursosMateriales extends ViewScopedRol implements Desarrollable {
    ...
    @Getter @Setter
    CartaNoAdeudoRolRecursosMateriales rol;
    ...
    public void init(){
        ...
        cargado = true;
        try{
            setVistaControlador(ControlEscolarVistaControlador.CARTA_NO_ADUEDO_RECURSOS_MATERIALES);
            ResultadoEJB<Filter<PersonalActivo>> resAcceso = ejb.validarRecursosMateriales(logon.getPersonal().getClave());//validar si es director
            //System.out.println("resValidacion = " + resAcceso.getValor().getEntity().getPersonal());

            ResultadoEJB<Filter<PersonalActivo>> resValidacion = ejb.validarJefeOficinaRecursosMateriales(logon.getPersonal().getClave());
           //System.out.println("resValidacion = " + resValidacion.getValor().getEntity().getPersonal());
            ...
            rol = new CartaNoAdeudoRolRecursosMateriales(filtro, personal);
...
//            System.out.println("tieneAcceso2 = " + tieneAcceso);
    ...
    public void  getCatalogoArea(){
        try{
            ResultadoEJB<List<CatalogoNoAdeudoArea>> resCatalogo= ejb.getCatalogoAduedobyArea(AreaCartaNoAdeudo.RECURSOS_MATERIALES);
    ...
    public void getListaDtoNoAdeudo(){
        try {
          //  System.out.println("CartaNoAdeudoDireccionCarrera.getListaDtoNoAdeudo Generacionseleccionada "+ rol.getGeneracionSelect());
            ...
            ResultadoEJB<List<DtoNoAdeudoEstudiante.NoAdeudoEstudianteGeneral>> resList= ejb.packRecursosMList(rol.getGeneracionSelect(),rol.getNivelEstudios(), AreaCartaNoAdeudo.RECURSOS_MATERIALES,rol.getRecursosM().getPersonal());
...
//        Object newValue = event.getNewValue();
        ...
        ResultadoEJB<DtoNoAdeudoEstudiante.ServiciosMateriales> resUpdate= ejb.updateNoAdeudoRecursosM(registroNew.getServiciosMateriales(),rol.getRecursosM().getPersonal());
    ...
    public void updateCarta(){
        try{
            rol.getEstudiantes().stream().forEach(e->{
                ResultadoEJB<DtoNoAdeudoEstudiante.ServiciosMateriales> resUpdate= ejb.updateNoAdeudoRecursosM(e.getServiciosMateriales(),rol.getRecursosM().getPersonal());
