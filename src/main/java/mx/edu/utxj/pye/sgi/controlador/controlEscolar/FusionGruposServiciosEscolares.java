...
@Named
@ViewScoped
public class FusionGruposServiciosEscolares extends ViewScopedRol implements Desarrollable {
...
@PostConstruct
 ...
 cargado = true;
            setVistaControlador(ControlEscolarVistaControlador.FUSION_GRUPOS_SERVICIOS_ESCOLARES);
            ResultadoEJB<Filter<PersonalActivo>> resAcceso = ejb.validarJefeServiciosEscolares(logonMB.getPersonal().getClave());//validar si es director
//            System.out.println("resAcceso = " + resAcceso);

            ResultadoEJB<Filter<PersonalActivo>> resValidacion = ejb.validarEncargadoServiciosEscolares(logonMB.getPersonal().getClave());
//            System.out.println("resValidacion = " + resValidacion);
            ...
            rol = new FusionGruposRolDirector(filtro, serviciosEscolares, serviciosEscolares.getAreaOficial());
...
//            System.out.println("tieneAcceso2 = " + tieneAcceso);
            ...
            rol.setServiciiosEscolares(serviciosEscolares);
            ResultadoEJB<EventoEscolar> resEvento = ejb.verificarEvento(rol.getServiciiosEscolares());
//            System.out.println("resEvento = " + resEvento);
            ...
            rol.setDivisiones(ejb.getAreasAcademica().getValor());
            rol.setAreaUniversidad(rol.getDivisiones().get(0));
            obtenerProgramasEductaivos();
            obtenerGruposProgramaEducativo();
            obtenerEstudiatesGrupoSeleccionado();
        }catch (Exception e){mostrarExcepcion(e); }
    }

    public void obtenerProgramasEductaivos(){
        rol.setProgramas(ejb.getProgramasEducativos(rol.getAreaUniversidad()).getValor());
        rol.setPrograma(
                ejb.getProgramasEducativos(
                        rol.getAreaUniversidad())
                        .getValor()
                        .get(0)
        );
        obtenerGruposProgramaEducativo();
