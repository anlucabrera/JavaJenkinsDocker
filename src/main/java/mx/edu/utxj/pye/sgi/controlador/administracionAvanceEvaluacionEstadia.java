...
@Named
@SessionScoped
public class administracionAvanceEvaluacionEstadia  implements Serializable{
    ...
    @Getter @Setter private List<ViewEstudianteAsesorAcademico> alumnosEncuestas;
    @Inject private LogonMB logonMB;
    @EJB private EjbAdministracionEncuesta ejbAdmEncuesta;
    @EJB private EjbAdministracionEvaluacionEstadia ejbAES;
    ...
    public void init(){
        ...
        try {
            ...
            esSecretario=false;
            planeacion = false;
            Logger.getLogger(administracionAvanceEvaluacionEstadia.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void mostrarAvanceEvaluacionEstadia() {
        try {
            ...
            alumnosEncuestas = ejbAES.obtenerEstudiantesAsesorAcademico();
            Comparador<EvaluacionEstadiaResultados> comparador = new ComparadorEvaluacionEstadia();
            alumnosEncuestas.forEach(x -> {
                try {
                    EvaluacionEstadiaResultados listaCompleta = ejbAES.obtenerResultadosEvaluacionPorAlumno(x.getMatricula());
            ...
            collect2.forEach((k2, v2) -> {
                ...
                collect = alumnosEncuestas.parallelStream().collect(Collectors.groupingBy(ViewEstudianteAsesorAcademico::getAbreviatura, Collectors.counting()));
                collect.forEach((k, v) -> {
                    siglas = k;
                    total = v;
