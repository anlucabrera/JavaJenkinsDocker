...
import mx.edu.utxj.pye.sgi.ejb.evaluaciones.EjbEvaluacionTutor2;
...
public class ServicioEvaluacionTutor2 implements EjbEvaluacionTutor2 {
    ...
    @EJB Facade2 f2;
    @EJB
    EJBAdimEstudianteBase ejbAdminAlumno;
...
//        System.out.println("mx.edu.utxj.pye.sgi.ejb.evaluaciones.ServicioEvaluacionTutor2.getSoloResultados() matricula :_ " + estudiante);
        TypedQuery<EvaluacionTutoresResultados> q = f.getEntityManager().createQuery("SELECT r FROM EvaluacionesTutoresResultados r WHERE r.evaluacionesTutoresResultadosPK.evaluacion=:evaluacion AND r.evaluacionesTutoresResultadosPK.evaluador=:evaluador", EvaluacionTutoresResultados.class);
        q.setParameter("evaluacion", evaluacion.getEvaluacion());
        q.setParameter("evaluador", estudiante);
        List<EvaluacionTutoresResultados> l = q.getResultList();
//        System.out.println("mx.edu.utxj.pye.sgi.ejb.evaluaciones.ServicioEvaluacionTutor2.getSoloResultados() lista de estudiante ejb : " + l);
        if(!l.isEmpty()){
            return l.get(0);
        }else{
            return null;
    ...
    public List<VistaEvaluacionesTutores> getListaTutores() {
        ...
        if (periodo.getResultList().isEmpty() || periodo.getResultList() == null) {
           // System.out.println("mx.edu.utxj.pye.sgi.ejb.evaluaciones.ServicioEvaluacionTutor2.getListaTutores() no se encontro ninguna lista de periodos");
        } else {
           // System.out.println("mx.edu.utxj.pye.sgi.ejb.evaluaciones.ServicioEvaluacionTutor2.getListaTutores() el periodo es  --> : " + periodo);
        }
