...
//    TODO: Validar roles, hacer la referencia del alcance si es general o específico
     ...
     *
     * @param notificacionCE Entidad que contiene los datos de la nueva
     * notificación y que será persistida
     * @return Devuelve la entidad ya persistida la cual ya contiene el id único
     * de la notificación nueva
     */
    ...
    /**
     * Método que permite la eliminación general de una notificación.Eliminando
     * de igual manera las imágenes asignadas a la notificación almacenadas en
     * el servidor Elimina enlaces asignados Elimina áreas asignadas a la
     * notificación
     *
     * @param notificacion Parámetro que contiene el identificador único de la
     * notificación que será eliminada
     * @return Devuelve VERDADERO cuando la operación ha sido ejecutada con
     * éxito, devuelve FALSO cuando hubo un error, ambos casos incluyen mensajes
     * de verificación
     */
    public ResultadoEJB<Boolean> eliminarNotificacion(@NonNull NotificacionesCe notificacion) {
        try {
            Integer notificacionID = notificacion.getNotificacion();
            em.remove(em.merge(notificacion));
            em.flush();
            NotificacionesCe notEncontrada = em.find(NotificacionesCe.class, notificacionID);
            if (notEncontrada == null) {
                return ResultadoEJB.crearCorrecto(Boolean.TRUE, "Se ha eliminado correctamente la notificación");
            } else {
    ...
    /**
     * Método que permite la actualización de información de una notificación en
     * específico.
     *
     * @param notificacionCe Entidad que contiene la información actualizada y
     * preparada para la modificación en base de datos
     * @return Devuelve la entidad actualizada y sincronizada con la base de
     * datos.
     */
    public ResultadoEJB<NotificacionesCe> editarNotificacion(@NonNull NotificacionesCe notificacionCe) {
        try {
            em.merge(notificacionCe);
            return ResultadoEJB.crearCorrecto(notificacionCe, "Se han actualizado los datos de la notificación: " + notificacionCe.getTituloPrincipal());
        } catch (Exception e) {
    ...
    /**
     * Método que es utilizado para consultar las últimas diez notificaciones
     * registradas, método ocupado desde que el usuario ingresa al módulo de
    ...
    public ResultadoEJB<List<NotificacionesCe>> consultarNotificacionesUltimosDiez() {
        try {
            ...
            if (!listaNotificaciones.isEmpty()) {
                ...
                return ResultadoEJB.crearErroneo(2, Collections.EMPTY_LIST, "Aún no se han registrado notificaciones.");
            }
        } catch (Exception e) {
    ...
    /**
     * Método que permite realizar una búsqueda personalizada de las
     * notificaciones registradas, la búsqueda es realizada con base a la fecha
     * de registro de la notificación
     *
     * @param fechaInicio Fecha de inicio seleccionada por el usuario desde la
     * interfaz
     * @param fechaFin Fecha de fin seleccionada por el usuario desde la
     * interfaz
     * @return Lista de entidades de tipo NotificacionesCe
     */
    public ResultadoEJB<List<NotificacionesCe>> consultaNotificacionesPorFechaRegistro(@NonNull Date fechaInicio, @NonNull Date fechaFin) {
        try {
            List<NotificacionesCe> listaNotificaciones = em.createQuery("SELECT n FROM NotificacionesCe n WHERE n.fechaRegistro BETWEEN :fechaInicio AND :fechaFin ORDER BY n.fechaRegistro DESC", NotificacionesCe.class)
                    ...
                    .getResultList();
            if (!listaNotificaciones.isEmpty()) {
                return ResultadoEJB.crearCorrecto(listaNotificaciones, "Listado de notificaciones registradas en la fecha seleccionada.");
            } else {
                return ResultadoEJB.crearErroneo(2, Collections.EMPTY_LIST, "No se ha registrado notificaciones en el periodo seleccionado.");
            }
        } catch (Exception e) {
    ...
    /**
     * Método que permite el registro de la asignación de un enlace a una
     * notificación.
     *
     * @param notificacionesEnlaces Entidad que contiene los datos del nuevo
     * enlace y que será persistido
     * @return Devuelve la entidad ya persistida la cual ya contiene el id único
     * del enlace asignado a la notificación
     */
    ...
    /**
     * Método que permite la eliminación del enlace de la notificación
     * seleccionada por el usuario
     *
     * @param notificacionEnlace Entidad que contiene el identificador único
     * para la eliminación
     * @return Devuelve VERDADERO cuando la operación es ejecutada con éxito,
     * Devuelve FALSO cuando no se ha podido ejecutar la operación o bien ocurre
     * un error
     */
    ...
    /**
     * Método que permite la actualización de información del enlace de la
     * notificación seleccionada
     *
     * @param notificacionEnlace Entidad que contiene la información actualizada
     * del enlace de la notificación
     * @return Devuelve la entidad actualizada y sincronizada con la base de
     * datos
     */
    ...
    /**
     * Método que permite la consulta de enlaces asignadas a una notificación
     * específica
     *
     * @param notificacion Entidad que contiene la información de la
     * notificación seleccionada por el usuario
    ...
    /**
     * Método que permite llenar en el DTO de NotificacionesAreas el listado de
     * Áreas que serán utilizadas para la asignación con notificaciones.
     *
    ...
    /**
     * Método que permite verificar si un área ya se encuentra asignada a la
     * notificación, con el fin de evitar un error de duplicidad de información
     *
     * @param notificacionArea Entidad que contiene los datos del área y la
     * notificación que serán ocupados para realizar la consulta a la base de
     * datos
     * @return Devuelve VERDADERO cuando se encuentra registro en base de datos,
     * Devuelve FALSO cuando ocurre un error en la consulta o bien no existe
    ...
    public ResultadoEJB<Boolean> verificaNotificacionArea(@NonNull NotificacionesAreas notificacionArea) {
        try {
            NotificacionesAreas na = em.createQuery("SELECT na FROM NotificacionesAreas na INNER JOIN na.notificacionesCe nce WHERE nce.notificacion = :notificacion AND na.notificacionesAreasPK.area = :area", NotificacionesAreas.class)
                    ...
                    .getSingleResult();
            if (na != null) {
                return ResultadoEJB.crearCorrecto(Boolean.TRUE, "Se ha encontrado un registro de la asignación de área con notificación");
            } else {
                return ResultadoEJB.crearErroneo(3, Boolean.FALSE, "No se ha encontrado registro de la asignación de área con notificación");
            }
     ...
     *
     * @param notificacionArea Contiene los parámetros necesarios para la
     * eliminación de una asignación de área específica
     * @return Devuelve VERDADERO si la eliminación ha sido realizada con éxito,
     * Devuelve FALSO si la verificación no es verdadera o ha surgido un error
    ...
    /**
     * Guarda o elimina la asignación de un área en una notificación, mediante
     * el método de verificación y eliminación
     *
     * @param notificacionArea Entidad que permitirá realizar la búsqueda y
     * determinar si será asignada o eliminada
     * @return Devuelve VERDADERO si la asignación es eliminada o asignada,
     * devuelve FALSO si en ambos casos no es posible realizar la operación de
     * guardar o eliminar
     */
...
//    Administración de imagenes que serán asignadas a la notificación
    /**
     * Método que permite la asignación de una o mas imágenes en una
     * notificación, incluye la copia del archivo en el servidor
     *
     * @param notificacionImagen Contiene los datos necesarios para identificar
     * quien es el usuario que sube la evidencia
     * @param archivos Contiene el listado de archivos que el usuario ha enviado
     * desde la interfaz
     * @return Devuelve un mapa que contiene la cantidad de correctos y el
     * numero de archivos que se han subido o guardado
     */
    ...
    /**
     * Método que permite la eliminación de una imagen asignada a una
     * notificación, eliminación del servidor y base de datos
     *
     * @param notificacionImagenCe Contiene el identificador único de la imagen
     * que se eliminará, ruta para eliminación del servidor y id para base de
     * datos
     * @return Devuelve VERDADERO si se ha detectado que el archivo ha sido
     * eliminado correctamente del servidor y en base de datos, devuelve FALSE
     * si se detecta un problema con la eliminación de ambas partes
     */
    ...
    /**
     * Método que permite la eliminación directa de todas las imágenes asignadas
     * a una notificación
     *
     * @param notificacionCe Contiene el id único para solo eliminar las
     * imágenes que sean pertenecientes a ella.
     * @return Devuelve VERDADERO si las evidencias fueron eliminadas
     * correctamente, devuelve FALSE si las evidencias no fueron eliminadas
     * correctamente
     */
