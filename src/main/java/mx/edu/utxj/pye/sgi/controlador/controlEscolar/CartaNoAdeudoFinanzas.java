...
import mx.edu.utxj.pye.sgi.dto.controlEscolar.CartaNoAdeudoRolFinanzas;
...
@Named
@ViewScoped
public class CartaNoAdeudoFinanzas extends ViewScopedRol implements Desarrollable {
    ...
    public void init(){
        ...
        cargado = true;
        try{
            setVistaControlador(ControlEscolarVistaControlador.CARTA_NO_ADUEDO_FINANZAS);
            ResultadoEJB<Filter<PersonalActivo>> resAcceso = ejb.validarIngresosPropios(logon.getPersonal().getClave());//validar si es director
...
//            System.out.println("resValidacion = " + resValidacion);
            ...
            PersonalActivo finanzas = filtro.getEntity();//ejbPersonalBean.pack(logon.getPersonal());
            rol = new CartaNoAdeudoRolFinanzas(filtro, finanzas);
            tieneAcceso = rol.tieneAcceso(finanzas);
...
//            System.out.println("tieneAcceso2 = " + tieneAcceso);
            ...
            rol.setPersonalActivo(finanzas);
            // ----------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public void  getCatalogoArea(){
        try{
            ResultadoEJB<List<CatalogoNoAdeudoArea>> resCatalogo= ejb.getCatalogoAduedobyArea(AreaCartaNoAdeudo.FINANZAS);
    ...
    public void getListaDtoNoAdeudo(){
        try {
            ...
            ResultadoEJB<List<DtoNoAdeudoEstudiante.NoAdeudoEstudianteGeneral>> resList= ejb.packFinanzasList(rol.getGeneracionSelect(),rol.getNivelEstudios(), AreaCartaNoAdeudo.FINANZAS,rol.getPersonalActivo().getPersonal());
...
//        Object newValue = event.getNewValue();
        ...
        ResultadoEJB<DtoNoAdeudoEstudiante.Finanzas> resUpdate= ejb.updateNoAdeudoFinanzas(registroNew.getFinanzas(),rol.getPersonalActivo().getPersonal());
    ...
    public void updateCarta(){
        try{
            rol.getEstudiantes().stream().forEach(e->{
                ResultadoEJB<DtoNoAdeudoEstudiante.Finanzas> resUpdate= ejb.updateNoAdeudoFinanzas(e.getFinanzas(),rol.getPersonalActivo().getPersonal());
    ...
    public void consultarListaPagos(DtoNoAdeudoEstudiante.NoAdeudoEstudianteGeneral matricula) {
        rol.setEstudanteSelect(matricula);
