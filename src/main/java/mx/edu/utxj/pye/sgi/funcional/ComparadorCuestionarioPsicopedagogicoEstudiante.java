...
public class ComparadorCuestionarioPsicopedagogicoEstudiante implements Comparador<CuestionarioPsicopedagogicoResultados> {
    ...
    public boolean isCompleto(CuestionarioPsicopedagogicoResultados resultados){
        ...
        if(resultados==null){ t=false; }
            ...
            //TODO:Comprueba que todas las preguntas obligatorias generales esten contestadas
            if(obligatoriasGenerales(resultados)==false){
                t=true;
                //TODO: Verfica si en la pregunta 16 respondio "Finados", en caso de que si, comprueba que haya sido respondida la pregunta 17
                if(resultados.getR16().equals("Finados")){
                    if (obligatoriaP17(resultados)==false){ t= true; }
                    else{ t= false;}
                }
                //TODO: Verifica si en la pregunta 19 respondio "Si", en caso de que si, verifica que la pregunta 20 y 21 hayan sido respondidas
                if(resultados.getR19().equals("Si")){
                    if(obligatoriaP20(resultados)==false){ t=true; }
                    else { t=false; }
                }
                //TODO: Verifica si en la pregunta 23 respondio "Si", en caso de que si, verifica que haya repondido la pregunta 24
                if(resultados.getR23().equals("Si")){
                    if(obligatoriaP24(resultados)==false){ t= true;
                    }else { t=false; }
                }
                //TODO:Verifica si en la pregunta 25 haya repondido "Si", en caso de que si, verifica que haya respondido las preguntas de la 26 -42
                if(resultados.getR25().equals("Si")){
                    if(obligatorias26(resultados)==false){ t= true;
                    }else { t=false; }
                }
                //TODO: Verifica si en la pregunta 43 haya repondido "Si", en caso de que si, verifica que haya respondido la pregunta 44
