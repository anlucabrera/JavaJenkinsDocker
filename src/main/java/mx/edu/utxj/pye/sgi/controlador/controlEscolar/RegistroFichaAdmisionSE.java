...
import mx.edu.utxj.pye.sgi.dto.controlEscolar.RegistroFichaAdmisionRolServiciosEscolares;
...
@ViewScoped
public class RegistroFichaAdmisionSE extends ViewScopedRol implements Desarrollable {
    ...
    @Getter @Setter RegistroFichaAdmisionRolServiciosEscolares rol;
    ...
    public void init(){
        ...
        cargado = true;
        setVistaControlador(ControlEscolarVistaControlador.REGISTRO_FICHA_SE);
        ResultadoEJB<Filter<PersonalActivo>> resAcceso = ejbRegistroFicha.validaSE(logonMB.getPersonal().getClave()); //Validar si pertenece departamento de Servicios Escolares
        if(!resAcceso.getCorrecto()){ mostrarMensajeResultadoEJB(resAcceso);return;}//cortar el flujo si no se pudo verificar el acceso

        ResultadoEJB<Filter<PersonalActivo>> resValidacion = ejbRegistroFicha.validaSE(logonMB.getPersonal().getClave());
        ...
        rol = new RegistroFichaAdmisionRolServiciosEscolares(filtro, serviciosEscolares, serviciosEscolares.getAreaOperativa());
        ...
        rol.setSoloLecPer(Boolean.TRUE);
        cargaDatos();
    ...
    /**
     * Metodo para realizar un registro sin adjuntar CURP
     */
    public void regSinArchivo(){
        try{
            rol.setSoloLecPer(Boolean.FALSE);
            rol.setStep(1);
            rol.setTab1(false);
            rol.setTab2(false);
            getEstadosMexico();
            ...
            if(resPais.getCorrecto()==true){
                ...
                if(resEstados.getCorrecto()==true){
                    rol.getPersonaD().setPaisOr(resPais.getValor());
        ...
        try{
            ...
            if(resPersona.getCorrecto()==true){
                ...
                if(rol.getPersonaD().getPersona().getIdpersona()==Integer.parseInt(rol.getPwdPer())){
                    ResultadoEJB<DtoAspirante.PersonaR> resPer= ejbRegistroFicha.getPersonaRSE(rol.getPersonaD().getPersona());
                    if(resPer.getCorrecto()==true){
                        rol.setPersonaD(resPer.getValor());
                        ResultadoEJB<DtoAspirante.PersonaR> resPersona2 =  ejbRegistroFicha.getPersonaRSE(resPersona.getValor());
