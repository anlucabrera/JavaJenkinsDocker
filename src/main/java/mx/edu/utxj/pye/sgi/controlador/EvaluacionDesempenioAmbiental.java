...
@Named
@ViewScoped
public class EvaluacionDesempenioAmbiental extends ViewScopedRol implements Desarrollable {
    @Getter @Setter private EvaluacionRolMultiple.EvaluacionRolPersonal rol;
    @Getter @Setter private Boolean finalizado = false;
    @EJB private EjbEvaluacionConocimientoCumplimiento ejbE;
    ...
    public void init() {
        try{
            ...
            ResultadoEJB<Filter<PersonalActivo>> resAcceso = ejbE.validarPersonal(logonMB.getPersonal().getClave()); //Validar si pertenece departamento de Servicios Escolares

            if(!resAcceso.getCorrecto()){ mostrarMensajeResultadoEJB(resAcceso);return;}//cortar el flujo si no se pudo verificar el acceso

            ResultadoEJB<Filter<PersonalActivo>> resValidacion = ejbE.validarPersonal(logonMB.getPersonal().getClave());
...
//           System.out.println(resEvento.getValor());
            ...
            rol.setClavePersona(personalActivo.getPersonal().getClave());
